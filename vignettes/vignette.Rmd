...
Within the SeuratObj example, there is a single clonotype with 923 copies across the single-cells. In order to not over-bias the nearest neighbor approximation with duplicate values, **clone.proportion** was created. This parameter allows you to select the relative proportion of the neighbors allocated to clones, i.e., **near.neighbor** = 30, **clone.proportion** = 0.4, the indexed nearest neighbors sharing the exact clonotype will be 12. The remaining 18 will be go to 18 randomly sampled cell from the next 18 nearest clonotypes. 
...
There are two additional functions to that will return the outputs of a single layer before the multi-network tranformation 1) ```aaReduction()``` and 2) ```distReduction()```. 
...
## Remove shared feature space

There is a high degree of overlap in the feature space of expanded clonotypes, not only in the adaptive receptor genes that form the clonotype, but also in potentially functional genes. In order to identify divergent clusters in shared clonotypes using ```clonalCommunity()```, we can use the Trex dimensions to identify clusters in order to then regress out closely associated clonotypes.
...
plot8 <- DimPlot(SeuratObj, reduction = "umap", group.by = "CTaa") + 
  NoLegend() + 
  scale_color_viridis(option="B", discrete = TRUE)
plot9 <- DimPlot(SeuratObj, reduction = "regressClone", group.by = "CTaa") + 
  NoLegend() + 
  scale_color_viridis(option="B", discrete = TRUE)
