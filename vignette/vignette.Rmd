...
```{r}
trex_example <- runTrex(trex_example, AA.method = "auto", 
                    AA.properties = "AF", 
                    nearest.method = "nn",
                    near.neighbor = 40,
                    reduction.name = "Trex.AF")

trex_example <- runTrex(trex_example, AA.method = "mean", 
                    AA.properties = "KF", 
                    nearest.method = "threshold",
...
```{r}
plot1 <- DimPlot(trex_example, reduction = "Trex.AF") + NoLegend()

plot2 <- DimPlot(trex_example, reduction = "Trex.KF") + NoLegend()
...
trex_example <- FindMultiModalNeighbors(
  trex_example, reduction.list = list("pca", "apca", "Trex.AF"), 
...
plot4 <- DimPlot(trex_example, group.by = "CTaa") + scale_color_manual(values = viridis_pal(option = "H")(length(unique(trex_example$CTaa)))) + NoLegend()
...
trex_example <- RunUMAP(trex_example, reduction = 'Trex.AF', dims = 1:30, 
