...
Trex has 2 major functions - the first being ```maTrex()```, which is the backbone of the algorithm and returns the encoded values based on the selection of variables. Unlike ```runTrex()``` below, ```maTrex()``` does not filter the input for only T cells with attached TCR data. 
...
plot8 <- DimPlot(trex_example, 
                 reduction = "rna.umap", 
                 group.by = "CTaa") + 
                 NoLegend() + 
                 scale_color_viridis(option="B", discrete = TRUE)

plot9 <- DimPlot(trex_example, 
