...
for (m in seq_along(chain)) {
    ...
    for (n in seq_along(AA.properties)) {
      ...
      if (n == 3) {
        min <- apply(array.reshape,2,function(x) min(x, na.rm=TRUE))
        max <- apply(array.reshape,2, function(x) max(x, na.rm=TRUE))
        min[is.infinite(min)] <- NA
        max[is.infinite(max)] <- NA
        range <- list(min = min, max = max)
        saveRDS(range, file = paste0(names(chain)[m], ".range.rds"))
      }
